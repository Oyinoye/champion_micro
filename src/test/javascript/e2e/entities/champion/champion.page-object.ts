...
export default class ChampionComponentsPage {
  createButton: ElementFinder = element(by.id('jh-create-entity'));
  deleteButtons = element.all(by.css('div table .btn-danger'));
  title: ElementFinder = element(by.id('champion-heading'));
  ...
  async goToPage(navBarPage: NavBarPage) {
    await navBarPage.getEntityPage('champion');
    await waitUntilAnyDisplayed([this.noRecords, this.table]);
    return this;
  }

  async goToCreateChampion() {
    await this.createButton.click();
    return new ChampionUpdatePage();
  }
  ...
  async deleteChampion() {
    ...
    expect(await isVisible(championDeleteDialog.deleteModal)).to.be.false;
  }
}
